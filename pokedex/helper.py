    ...
    try:
        ...
        for pokemon_entry in pokemon_entries:
            ...
            if isinstance(pokemon_entry, dict):
                ...
                logging.debug(
                    f"Warning: Invalid Pokémon entry structure under key '{key}': {pokemon_entry}"
                )
                continue

            if not pokemon_name:
                logging.debug(
                    f"Warning: Could not find Pokémon name in entry under key '{key}': {pokemon_entry}"
                )
                continue
            ...
            if key == "pokemon_encounters":
                ...
                for version_detail in version_details:
                    encounter_info = {
                        ...
                        "method": version_detail["encounter_details"][0]["method"][
                            "name"
                        ],
                        "max_level": version_detail["encounter_details"][0][
                            "max_level"
                        ],
                        "min_level": version_detail["encounter_details"][0][
                            "min_level"
                        ],
            ...
            if "sprites" in pokemon:
                ...
                entry_number = pokemon.get(
                    "entry_number"
                )  # We ensure entry_number is required
                if entry_number is None:
                    official_artwork = official_artwork
                else:
                    official_artwork = pokedex.get_official_artwork(
                        pokemon_name, official_artwork, entry_number
                    )
                ...
                logging.debug(
                    f"No sprites found for Pokémon '{pokemon_name}' under key '{key}'"
                )
